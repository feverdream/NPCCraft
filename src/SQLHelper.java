...
public class SQLHelper {
    ...
    public static void Connect() {
        try {
            String uname = NPCCraft.getCfgParser().getStringValue("username");
            String pword =NPCCraft.getCfgParser().getStringValue("password");
            String port  = NPCCraft.getCfgParser().getStringValue("port");
            String db    = NPCCraft.getCfgParser().getStringValue("database");
            String serv  = NPCCraft.getCfgParser().getStringValue("server");
    ...
    public String sanitizeInput(final String value) {
        ...
        return value.replaceAll("&", "&amp;").replaceAll("&#x73;", "&amp;").replaceAll("<", "&lt;").replaceAll(
            ">", "&gt;").replaceAll("\\(", "&#40;").replaceAll("\\)", "&#41;").replaceAll("'", "&#x27").replaceAll(
            "\"", "&quot;").replaceAll("/", "&#x2F;").replaceAll("%", "&#37;").replaceAll(
            "(?i)eval\\((.*)\\)", "[xss-eval]").replaceAll("(?i)script", "[xss-script]").replaceAll(
            "(?i)[\\\"\\\'][\\s]*javascript:(.*)[\\\"\\\']", "\"\"").replaceAll(
            "(?i)<img|><script|script|%3cscript%3cimg|<script|<img|<script|<img|<script|<img|%0d", "").replaceAll(
            "(?i)(select|grant|delete|insert|drop|alter|replace|truncate|update|create|rename|describe|union)",
            "[xsql]");
